...
private: // Phosphenes
	...
	uint32_t				m_nCameraFrameWidth, m_nCameraFrameHeight;
	uint32_t				m_nCameraFrameBufferSize;
	cv::Mat                 image, img_ph_Right, img_ph_Left, img_show;
	std::vector<cv::Mat>    imgBuffer, poseBuffer;
	uint32_t                pnWidth, pnHeigth;
	vr::EVRTrackedCameraError nCameraError;

	Mat plotImgRight, plotImgLeft, sprite, reference_sprite;
	Mat fosfenosRight, fosfenosLeft;
	Mat K;
	...
	int x0, y0;
	int X0, Y0;
	//Mat srcbn1Right, srcbn8normalRight, srcbn1Left, srcbn8normalLeft;
	int grid_mode = 1;
	int N_fos = 1024;
	...
	double size_sprite;
	//Temporal model
	cv::Mat LUT, mat, casoRight, casoLeft, ampVectorRight, ampVectorLeft, matriz_imagen, imgRight; 
...
{
	//creación del vector de salida vacío
...
bool CMainApplication::ImageIntoPhosohenes() {
	...
	nCameraError = m_pVRTrackedCamera->GetVideoStreamFrameBuffer(m_hTrackedCamera, vr::VRTrackedCameraFrameType_Undistorted, image.data, m_nCameraFrameBufferSize, &frameHeader, sizeof(frameHeader));
	...
	cv::Mat srcbn1Right = SingleViewToPhosphenes(image, 420, 1188, 12 * 32, 12 * 32);
	cv::Mat srcbn1Left = SingleViewToPhosphenes(image, 420, 268, 12 * 32, 12 * 32);
	
	cv::Mat srcbn8normalRight = cv::Mat::zeros(srcbn1Right.size(), srcbn1Right.type());
	cv::Mat srcbn8normalLeft = cv::Mat::zeros(srcbn1Right.size(), srcbn1Right.type());
	for (int i = 0; i < srcbn1Right.rows; ++i) {
		for (int j = 0; j < srcbn1Right.cols; ++j) {
			...
			srcbn8normalLeft.at<unsigned char>(i, j) = srcbn1Left.at<unsigned char>(i, j) / 32;
		}
	}
	...
	plotImgRight.setTo(Vec3b(0, 0, 0));
	plotImgLeft.setTo(Vec3b(0, 0, 0));
	if (TemporalMode) {
		...
		for (int i = 0; i < XiCam.cols; ++i) {
			...
			if ((k >= 0) && (k < srcbn8normalLeft.rows) && (w >= 0) && (w < srcbn8normalLeft.cols) && (srcbn8normalLeft.at<unsigned char>(k, w) > grayThreshold2)) {
				ampVectorLeft.at<unsigned char>(0, i) = 2;
			}
			else if ((k >= 0) && (k < srcbn8normalLeft.rows) && (w >= 0) && (w < srcbn8normalLeft.cols) && (srcbn8normalLeft.at<unsigned char>(k, w) > grayThreshold1)) {
				ampVectorLeft.at<unsigned char>(0, i) = 1;
			}
			else {
				ampVectorLeft.at<unsigned char>(0, i) = 0;
			}

		}
		std::vector<int> phospheneFlagRight(phosphenesPos.cols);
		std::vector<int> phospheneFlagLeft(phosphenesPos.cols);
		phospheneFlagRight = TemporalModel(ampVectorRight, casoRight, phospheneFlagRight.size(), N_levels, maxVal, minVal, LUT);
		phospheneFlagLeft = TemporalModel(ampVectorLeft, casoLeft, phospheneFlagLeft.size(), N_levels, maxVal, minVal, LUT);
		visualize(phospheneFlagRight, plotImgRight, phosphenesPos_vis, sprites, sprite);
		visualize(phospheneFlagLeft, plotImgLeft, phosphenesPos_vis, sprites, sprite);
	}
	else {
		for (int i = 0; i < XiCam.cols; ++i) {
			...
			ampVectorLeft.at<unsigned char>(0, i) = srcbn8normalLeft.at<unsigned char>(k, w);

		}
		visualize(ampVectorRight, plotImgRight, phosphenesPos_vis, sprites, sprite);
		visualize(ampVectorLeft, plotImgLeft, phosphenesPos_vis, sprites, sprite);
	}
	
	// Pushing away calibrated image to have perspective in VR device. x is the calibration factor
	uint32_t m_right = (plotImgRight.rows * (x_right - 1) / 2); //distance between left screen limit and beggining of the picture
	uint32_t n_right = (plotImgRight.cols * (x_right - 1) / 2); //distance between top screen limit and beggining of the picture
	uint32_t m_left = (plotImgLeft.rows * (x_left - 1) / 2); //distance between left screen limit and beggining of the picture
	uint32_t n_left = (plotImgLeft.cols * (x_left - 1) / 2); //distance between top screen limit and beggining of the picture
	for (uint32_t a = 0; a < plotImgRight.cols; a++) {
		for (uint32_t b = 0; b < plotImgRight.rows; b++) {
			img_ph_Right.at<cv::Vec3b>(a + n_right, b + m_right) = plotImgRight.at<cv::Vec3b>(a, b);
			img_ph_Left.at<cv::Vec3b>(a + n_left, b + m_left) = plotImgLeft.at<cv::Vec3b>(a, b);
		}
	}
	img_show.create(img_ph_Right.rows, img_ph_Right.cols, CV_8UC1);
	img_show = img_ph_Right;
	...
	if (sleepFlag)
		Sleep((DWORD)(10 - time_span.count()));
...
void CMainApplication::RenderFrame()
{
	if (m_pHMD)
	{
		...
		cv::rotate(img_ph_Right, img_ph_Right, cv::ROTATE_90_COUNTERCLOCKWISE);
		cv::rotate(img_ph_Left, img_ph_Left, cv::ROTATE_90_COUNTERCLOCKWISE);
		

		// Converting into textures and submitting (into VR screens and in a window in the computer screen)
		image_tex_Left = matToTexture(img_ph_Right, GL_LINEAR, GL_LINEAR, GL_CLAMP, iniciogl, image_tex_Left);
		vr::Texture_t leftEyeTexture = { (void*)(uintptr_t)image_tex_Left, vr::TextureType_OpenGL, vr::ColorSpace_Gamma };
		vr::VRCompositor()->Submit(vr::Eye_Left, &leftEyeTexture);
		glFinish();
		image_tex_Right = matToTexture(img_ph_Left, GL_LINEAR, GL_LINEAR, GL_CLAMP, iniciogl, image_tex_Right);
		vr::Texture_t rightEyeTexture = { (void*)(uintptr_t)image_tex_Right, vr::TextureType_OpenGL, vr::ColorSpace_Gamma };
		vr::VRCompositor()->Submit(vr::Eye_Right, &rightEyeTexture);
		iniciogl = false;

		cv::flip(img_ph_Left, img_ph_Left, 0);
		cv::imshow("IMAGEN", img_ph_Left);
		cv::flip(img_ph_Right, img_ph_Right, 0);
		cv::imshow("IMAGEN", img_ph_Right);
		cv::waitKey(1);
	...
	// Clear
	{
		// Make sure the glFinish waits for the entire present to complete, not just the submission
		// of the command. Do a clear so the glFinish will wait fully for the swap.
...
bool CMainApplication::Initialitation() {
	...
	char buffer[128];
	m_pHMD->GetStringTrackedDeviceProperty(vr::k_unTrackedDeviceIndex_Hmd, vr::Prop_CameraFirmwareDescription_String, buffer, sizeof(buffer), &propertyError);
	...
	std::string txtFileName = "C:/Users/ivea/Desktop/Prueba_Maria/open/openvr/samples/bin/win64/prothesis_simulator.cfg"; // Reemplaza "archivo.csv" con la ruta de tu archivo CSV
	...
	plotImgRight = cv::Mat::zeros(cv::Size(384, 384), 16);
	plotImgLeft = cv::Mat::zeros(cv::Size(384, 384), 16);

	X0 = plotImgRight.cols / 2;
	fx = 196;
	fy = 189;

	Y0 = plotImgRight.rows / 2;
	K = (Mat_<double>(3, 3) << fx, 0, X0, 0, fy, Y0, 0, 0, 1);
	f = (fx + fy) / 2;

	img_ph_Right = cv::Mat::zeros(cv::Size(plotImgRight.rows * x_right, plotImgRight.cols * x_right), 16);
	img_ph_Left = cv::Mat::zeros(cv::Size(plotImgLeft.rows * x_left, plotImgLeft.cols * x_left), 16);


	// Phosphene Matrix configuration
	deltaX = computeDeltaFromNumberOfPhosphenes(plotImgRight.rows, plotImgRight.cols, N_fos);
	deltaY = deltaX;
	rExt = sqrt(X0 * X0 + Y0 * Y0);
	genPhosphenesPos(plotImgRight.rows, plotImgRight.cols, rExt, X0, Y0, deltaX, deltaY, grid_mode, noise_stddev, phosphenesPos, phosphenesPos_vis, XiCam, K);
	...
	if (TemporalMode) {
		...
		std::string csvFileName = "C:/Users/ivea/Desktop/Prueba_Maria/open/openvr/samples/bin/win64/tabulacion3.csv"; // Reemplaza "archivo.csv" con la ruta de tu archivo CSV
		//int columnIndex = 0; // Índice de la columna a leer (comenzando desde 0)
	...
	ampVectorLeft = cv::Mat::zeros(1, phosphenesPos.cols, CV_8U);
	grayThreshold1 = 1;
	grayThreshold2 = 6;

	m_pHMD->GetRecommendedRenderTargetSize(&pnWidth, &pnHeigth);
}
